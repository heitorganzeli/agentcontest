...
//			temp = Search.getNodeLocation(path.get(n-8));
	//	} else 
		if(n+1 > model.getCorral().center().distanceChebyshev(from.getLocation(model)) 
				&& n>4){

			temp = Search.getNodeLocation(path.get(4));
//			printNodes(path);
		}else{
	...
	private void printNodes(List<Nodo> path){
		for(Nodo no: path){
			System.out.println("CLUSTER ___ "+ Search.getNodeLocation(no));
		}
	}
